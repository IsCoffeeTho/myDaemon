...
class commandHandler extends batchableCommandHandler {
	...
	binds() {
		return JSON.parse(this.#issue("binds", "-j"));
	}
	layers() {
		return JSON.parse(this.#issue("layers", "-j"));
	}
	splash() {
		return JSON.parse(this.#issue("splash", "-j"));
	}
	locked() {
		return JSON.parse(this.#issue("locked", "-j"));
	}
	submap() {
		return JSON.parse(this.#issue("submap", "-j"));
	}
	clients() {
		return JSON.parse(this.#issue("clients", "-j"));
	}
	devices() {
		return JSON.parse(this.#issue("devices", "-j"));
	}
	layouts() {
		return JSON.parse(this.#issue("layouts", "-j"));
	}
	version() {
		return JSON.parse(this.#issue("version", "-j"));
	}
	monitors() {
		return JSON.parse(this.#issue("monitors", "-j"));
	}
	cursorpos() {
		return JSON.parse(this.#issue("cursorpos", "-j"));
	}
	instances() {
		return JSON.parse(this.#issue("instances", "-j"));
	}
	workspaces() {
		return JSON.parse(this.#issue("workspaces", "-j"));
	}
	animations() {
		return JSON.parse(this.#issue("animations", "-j"));
	}
	rollinglog() {
		return JSON.parse(this.#issue("rollinglog", "-j"));
	}
	configerrors() {
		return JSON.parse(this.#issue("configerrors", "-j"));
	}
	descriptions() {
		return JSON.parse(this.#issue("descriptions", "-j"));
	}
	activewindow() {
		return JSON.parse(this.#issue("activewindow", "-j"));
	}
	workspacerules() {
		return JSON.parse(this.#issue("workspacerules", "-j"));
	}
	activeworkspace() {
		return JSON.parse(this.#issue("activeworkspace", "-j"));
	}
	getoption(option: string) {
		return JSON.parse(this.#issue("getoption", option, "-j"));
	}
	decorations(window: string) {
		return JSON.parse(this.#issue("decorations", window, "-j"));
	}
